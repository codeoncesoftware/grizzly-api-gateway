    ...
    stages {
	  ...
	  stage('Docker Stage for latest version') {
	   ...
	   stage('Update ECS Fargate Service CI') {
	   		when { branch "develop" }
	   		steps {
	   	 		 sh 'aws ecs update-service --service codeonce-grizzly-gateway-service --cluster codeonce-fargate-ci --region eu-central-1 --force-new-deployment'
	   		}
	   }
	   stage('Update ECS Fargate Service PROD') {
	   		when { branch "master" }
	   		steps {
	   	 		 sh 'aws ecs update-service --service codeonce-grizzly-gateway-service-prod --cluster codeonce-fargate-prod --region eu-central-1 --force-new-deployment'
	   		}
	   }
	   stage('Mirroring') {
            when { branch "master" }
